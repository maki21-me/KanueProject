  ...
  return (
    <div className='shadow-lg bg-white dark:bg-gray-900 dark:text-white duration-200 fixed w-full z-50'>
      <div className='container py-3 sm:py-0 flex justify-between items-center'>
        {/* Logo */}
        <Link to="/" className='font-logo text-3xl sm:text-4xl text-primary drop-shadow-xl tracking-wider italic'>
          Canoe
        </Link>
              ...
              if (menu.isDropdown) {
                return (
                  <li key={menu.id} className='group relative cursor-pointer'>
                    <Link to={menu.link} className='flex h-[72px] items-center gap-[2px]'>
                      {menu.name}
      ...
      {/* Mobile Menu */}
      <div
        className={`sm:hidden fixed top-0 right-0 w-64 h-full bg-white dark:bg-gray-900 shadow-lg transform ${mobileMenuOpen ? "translate-x-0" : "translate-x-full"} transition-transform duration-300 z-40`}
      >
        {/* X Button */}
